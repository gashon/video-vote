...
def start_page():
    ...
    st.warning(f"Please only use Google Chrome as your browser if you are not already doing so!")
    ...
    checked[1] = st.checkbox("I will be thoughtful and make my best judgment before finalizing any decisions.")
...
def show_videos_page(eval_id):
    ...
    if CRITERIA[criteria_id][0] in ["Text Following", "Character Emotions"]:
        with open(osp.join(VIDEO_ROOT, MODEL_LIST[0], f"prompt-{prompt_id}/summary.txt")) as f:
            prompt = f.read()
        st.markdown("#### Prompt:")
